...
// 網頁版麻將遊戲控制函數
async function startMahjongInPanel() {
    try {
        ...
        if (data.success) {
            ...
            showMahjongMessage('遊戲開始！請摸牌。', 'success');
        } else {
            showMahjongMessage('遊戲開始失敗：' + data.error, 'danger');
        }
    } catch (error) {
        showMahjongMessage('網絡錯誤：' + error.message, 'danger');
    }
}

async function drawTileInPanel() {
    try {
        ...
        if (data.success) {
            renderPlayerHand(data.player_hand);
            updateGameStatus(data.remaining_tiles, '你');
            showMahjongMessage('摸到：' + data.new_tile + '，請選擇要打出的牌。', 'info');
            document.getElementById('drawBtn').disabled = true;
        } else {
            showMahjongMessage('摸牌失敗：' + data.error, 'danger');
        }
    } catch (error) {
        showMahjongMessage('網絡錯誤：' + error.message, 'danger');
    }
...
function renderPlayerHand(hand) {
    ...
    hand.forEach((tile, index) => {
        ...
        tileElement.textContent = tile;
        tileElement.style.cssText = `
            width: 28px; height: 35px; background: #fff; border: 1px solid #333; 
            display: flex; align-items: center; justify-content: center; 
        ...
        // 添加麻將牌圖案樣式
        if (tile.includes('萬')) {
            tileElement.style.color = '#d73527';
        } else if (tile.includes('條')) {
            tileElement.style.color = '#16a34a';
        } else if (tile.includes('筒')) {
            tileElement.style.color = '#2563eb';
        }
        
        tileElement.onclick = () => discardTileFromPanel(index);
        container.appendChild(tileElement);
    });
}

// 打出牌（網頁版）
async function discardTileFromPanel(tileIndex) {
    try {
        ...
        if (data.success) {
            ...
            showMahjongMessage('你打出：' + data.discarded_tile, 'info');
            
            // 開始電腦回合
            setTimeout(computerTurnInPanel, 1000);
        } else {
            showMahjongMessage('打牌失敗：' + data.error, 'danger');
        }
    } catch (error) {
        showMahjongMessage('網絡錯誤：' + error.message, 'danger');
    }
...
// 電腦回合（網頁版）
async function computerTurnInPanel() {
    try {
        ...
        if (data.success) {
            ...
            showMahjongMessage('電腦' + data.computer_player + '打出：' + data.discarded_tile, 'info');
            ...
            if (data.available_actions && data.available_actions.length > 0) {
                ...
                // 輪到玩家
                document.getElementById('drawBtn').disabled = false;
                showMahjongMessage('輪到你了，請摸牌。', 'success');
            } else {
                // 繼續電腦回合
                setTimeout(computerTurnInPanel, 1500);
            }
        } else {
            showMahjongMessage('電腦回合錯誤：' + data.error, 'danger');
        }
    } catch (error) {
        showMahjongMessage('網絡錯誤：' + error.message, 'danger');
    }
...
// 顯示遊戲訊息（網頁版）
function showMahjongMessage(message, type) {
...
function renderDiscardedTiles(tiles) {
    ...
    tiles.forEach(tile => {
        ...
        tileElement.textContent = tile;
        tileElement.style.cssText = `
            width: 20px; height: 26px; background: #ffffcc; border: 1px solid #333; 
            display: flex; align-items: center; justify-content: center; 
            font-size: 9px; font-weight: bold; border-radius: 1px; color: #000;
        `;
...
function renderComputerHands(handCounts) {
    for (let i = 0; i < 3; i++) {
        ...
        for (let j = 0; j < handCounts[i]; j++) {
            ...
            tileElement.style.cssText = `
                width: 20px; height: 26px; background: #8B4513; border: 1px solid #333; 
                display: flex; align-items: center; justify-content: center; 
                font-size: 10px; border-radius: 1px; margin: 1px;
            `;
            container.appendChild(tileElement);
        }
    }
...
function updateGameStatus(remainingTiles, currentPlayer) {
    ...
    if (remainingElement) remainingElement.textContent = remainingTiles;
...
async function executeAction(action) {
    try {
        ...
        if (data.success) {
            ...
            if (data.discarded_tiles) {
                renderDiscardedTiles(data.discarded_tiles);
            }
            if (data.melded_tiles) {
                renderMeldedTiles(data.melded_tiles);
            }
            
            showMahjongMessage(data.message || `執行動作：${action}`, 'success');
            
            // 隱藏動作提示
            document.getElementById('actionPrompt').style.display = 'none';
            
            // 檢查是否需要等待玩家動作
            if (data.waiting_for_discard) {
                showMahjongMessage('請選擇要打出的牌', 'info');
            } else if (data.game_over) {
                showMahjongMessage(data.result || '遊戲結束', 'primary');
            } else {
                // 繼續遊戲流程
                setTimeout(() => {
                    if (data.next_player !== 0) {
                        computerTurnInPanel();
                    }
                }, 1000);
            }
        } else {
            showMahjongMessage('動作失敗：' + data.error, 'danger');
        }
    } catch (error) {
        showMahjongMessage('網絡錯誤：' + error.message, 'danger');
    }
...
async function passAction() {
    try {
        ...
        const data = await response.json();
        if (data.success) {
            showMahjongMessage('過牌', 'info');
            
            // 隱藏動作提示
            document.getElementById('actionPrompt').style.display = 'none';
            
            // 繼續遊戲流程
            setTimeout(computerTurnInPanel, 500);
        } else {
            showMahjongMessage('過牌失敗：' + data.error, 'danger');
        }
    } catch (error) {
        showMahjongMessage('網絡錯誤：' + error.message, 'danger');
    }
...
// 顯示動作提示（網頁版）
function showActionPrompt(actions, tile) {
    const promptElement = document.getElementById('actionPrompt');
    if (promptElement) {
        promptElement.style.display = 'block';
        showMahjongMessage(`可對 ${tile} 執行：${actions.join(', ')}`, 'info');
    }
}
...
// 渲染組合牌
function renderMeldedTiles(meldedTiles) {
    const container = document.getElementById('playerMelded');
    if (!container) return;
    
    container.innerHTML = '';
    meldedTiles.forEach(meld => {
        ...
        meld.forEach(tile => {
            ...
            tileElement.style.cssText = `
                ...
                display: flex; align-items: center; justify-content: center; 
                font-size: 10px; font-weight: bold; border-radius: 2px; color: #000;
            `;
...
window.loadMahjongGame = loadMahjongGame;
window.startMahjongInPanel = startMahjongInPanel;
window.drawTileInPanel = drawTileInPanel;
window.executeAction = executeAction;
window.showActionPrompt = showActionPrompt;
window.hideActionPrompt = hideActionPrompt;
